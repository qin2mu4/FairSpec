...
class _MyMapDatasetFetcher(_BaseDatasetFetcher):
    def fetch(self, possibly_batched_index):
        ...
        for data_one in data:
            prompt = data_one['prompt'].split(':')[0]
            gender_one = [0, 0]
            if 'female' in prompt:
                gender_one[sens_label_dict['female']] = 1
            elif 'male' in prompt:
                gender_one[sens_label_dict['male']] = 1
            gender.append(gender_one)
            gender.append(gender_one)
            age_one = [-1]
            if 'teenager' in prompt:
                age_one = [0]
            elif 'young adult' in prompt:
                age_one = [1]
            elif 'middle-aged' in prompt:
                age_one = [2]
            elif 'elderly person' in prompt:
                age_one = [3]
            age.append(age_one)
            age.append(age_one)
        result['gender_label'] = torch.tensor(gender, dtype=torch.int)
        result['age_label'] = torch.tensor(age, dtype=torch.int)
        return result

class _MySingleProcessDataLoaderIter(_SingleProcessDataLoaderIter):
    def __init__(self, loader):
        super().__init__(loader)
        self._dataset_fetcher = _MyMapDatasetFetcher(
            self._dataset_fetcher.dataset, self._dataset_fetcher.auto_collation, self._dataset_fetcher.collate_fn, self._dataset_fetcher.drop_last
        )
...
class Trainer:
    ...
    def _get_dataloader(
        ...
        kwargs = {
            'num_workers': dataloader.base_dataloader.num_workers,
            'collate_fn': dataloader.base_dataloader.collate_fn,
            'pin_memory': dataloader.base_dataloader.pin_memory,
            'timeout': dataloader.base_dataloader.timeout,
            'worker_init_fn': dataloader.base_dataloader.worker_init_fn,
            'multiprocessing_context': dataloader.base_dataloader.multiprocessing_context,
            'generator': dataloader.base_dataloader.generator,
            'prefetch_factor': dataloader.base_dataloader.prefetch_factor,
            'persistent_workers': dataloader.base_dataloader.persistent_workers,
            'pin_memory_device': dataloader.base_dataloader.pin_memory_device,
        }
        dataloader.base_dataloader = MyDataLoader(dataset=dataloader.base_dataloader.dataset,
                                                  batch_sampler=dataloader.base_dataloader.batch_sampler,
                                                  **kwargs)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                p = float(update_step + epoch * len_dataloader) / num_train_epochs / len_dataloader
                alpha = 2. / (1. + np.exp(-10 * p)) - 1
                for i in range(len(batch_samples)):
                    batch_samples[i]['alpha'] = alpha
            ...
            os.makedirs(args.output_dir + '/final_save', exist_ok=True)
            save_file(model.state_dict(), args.output_dir + f'/final_save/peft_model_{epoch+1}.safetensors')
