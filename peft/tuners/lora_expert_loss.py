        ...
        for key in key_list:
            ...
            if target_module_found: # here
                ...
                if isinstance(target, torch.nn.Linear) and self.peft_config.enable_lora is None:
                    new_module = Linear(target.in_features, target.out_features,
...
#
#     def forward(self, token_embeddings):  # [B, L, H]
#         attn_output, _ = self.attn(token_embeddings, token_embeddings, token_embeddings)
#         # cls_output = attn_output[:, 0, :]  # use [CLS]
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            if len(self.sens_size) > 0:
                self.lora_route = nn.Sequential(
                    nn.Linear(in_features + 256, 128),
                    nn.ReLU(),
                    nn.Linear(128, self.lora_num)
                )
            else:
                self.lora_route = nn.Sequential(
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A0"):
            ...
            for m in self.lora_route.modules():
    ...
    def forward(self, x: torch.Tensor, router_pred, task_types=None, reture_router=False):
        route_save = None
        fair_e_result = None
        if self.disable_adapters:
            ...
            if self.r > 0:
                router_in = x
                if len(self.sens_size) > 0:
                    router_in = torch.cat([x, router_pred], dim=-1)
                route_weight = nn.functional.softmax(self.lora_route(router_in), dim=-1, dtype=torch.float32).to(result.dtype)
                if reture_router:
                    route_save = route_weight.detach().cpu()
                for i in self.fair_e:
                    if  i in self.ablation:
                        continue
                    lora_result = getattr(self, f"lora_B{i}")(getattr(self, f"lora_A{i}")(self.lora_dropout(x)))
                    result = result + torch.unsqueeze(route_weight[:, :, i], -1) * lora_result * self.scaling
                fair_e_result = result

                for i in range(self.lora_num):
                    if i in self.fair_e or i in self.ablation:
                        continue
                    lora_result = getattr(self, f"lora_B{i}")(getattr(self, f"lora_A{i}")(self.lora_dropout(x)))
                    result = result + torch.unsqueeze(route_weight[:, :, i], -1) * lora_result * self.scaling
        ...
        return result, fair_e_result, route_save
