...
def _prepare_lora_config(peft_config, model_config):
    ...
    return peft_config # 训练时上面的if一个也不进
...
def get_peft_model(model, peft_config):
    ...
    if peft_config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys(): # 这里不进
        peft_config = _prepare_lora_config(peft_config, model_config)
        return PeftModel(model, peft_config)
    if not isinstance(peft_config, PromptLearningConfig): # -------------> here
        peft_config = _prepare_lora_config(peft_config, model_config) # 进这里
    else:
